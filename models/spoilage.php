<?php
    ...
    class spoilage extends db{
        ...
        public function getspoilagecategories(){
            $sql="CALL `sp_getspoilagecategory`()";
            return $this->getJSON($sql);
        }

        public function savespoilage($id,$categoryid,$productid,$quantity,$narration,$storecategory,$storeid){
            $sql="CALL `sp_savespoilage`({$id},{$categoryid},{$productid},{$quantity},'{$narration}','{$storecategory}',{$storeid},{$_SESSION['userid']})";
            $this->getData($sql);
            return "success";
        }
        ...
        public function filterspoilage($startdate,$enddate,$categoryid,$productid){
            ...
            $sql="CALL `sp_filterspoilage`('{$startdate}','{$enddate}',{$categoryid},{$productid})";
            // echo $sql."<br/>";
            return $this->getJSON($sql);
        }
    }
