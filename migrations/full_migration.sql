...
CREATE PROCEDURE `spchangeuserpassword`(
    ...
    IN p_changepasswordonlogon TINYINT
)
BEGIN
    UPDATE `user` 
    SET `password` = p_userpassword, 
        `salt` = p_salt,
        `changepasswordonlogon` = p_changepasswordonlogon 
    WHERE clientid = p_clientid AND userid = p_userid;
END $$

DROP PROCEDURE IF EXISTS `spcheckcategory` $$
CREATE PROCEDURE `spcheckcategory`(IN clientid INT, IN p_id INT, IN p_field VARCHAR(50), IN p_value VARCHAR(50))
BEGIN
    IF p_field = 'categoryname' THEN
        SELECT * FROM `categories` WHERE clientid = p_clientid AND `categoryid` <> p_id AND `categoryname` = p_value;
    ELSEIF p_field = 'prefix' THEN
        SELECT * FROM `categories` WHERE clientid = p_clientid AND `categoryid` <> p_id AND `prefix` = p_value;
    END IF;
END $$
...
CREATE PROCEDURE `spcheckcustomername`(IN clientid INT, IN p_id INT, IN p_name VARCHAR(50))
BEGIN
    SELECT * FROM `customers` WHERE clientid = p_clientid AND `customerid` <> p_id AND `customername` = p_name;
END $$
...
CREATE PROCEDURE `spcheckproduct`(IN clientid INT, IN p_id NUMERIC, IN p_valuetocheck VARCHAR(50), IN p_category VARCHAR(50))
BEGIN
    IF p_category='code' THEN
        SELECT * FROM `products` WHERE clientid = p_clientid AND `productid` <> p_id AND `itemcode` = p_valuetocheck;
    ELSE
        SELECT * FROM `products` WHERE clientid = p_clientid AND `productid` <> p_id AND `itemname` = p_valuetocheck;
    END IF;
END $$

DROP PROCEDURE IF EXISTS `spcheckreferenceno` $$
CREATE PROCEDURE `spcheckreferenceno`(IN p_branchid INT, IN p_modeid INT, IN p_reference VARCHAR(50))
BEGIN
    SELECT * FROM `possalespayments` WHERE branchid = p_branchid AND `paymentmode` = p_modeid AND `reference` = p_reference;
END $$
...
CREATE PROCEDURE `spchecksupplierdeliverynotenumber`(IN p_branchid INT, IN p_supplierid INT, IN p_deliverynoteno VARCHAR(50))
BEGIN
    SELECT * FROM `goodsreceived` WHERE branchid = p_branchid AND `supplierid` = p_supplierid AND `deliverynono` = p_deliverynoteno;
END $$
...
CREATE PROCEDURE `spcheckuser`(`$userid` INT, `$checkfield` VARCHAR(50), `$checkvalue` VARCHAR(50))
BEGIN
...
CREATE PROCEDURE `spdeletecategory`(IN clientid INT, IN p_id INT, IN p_userid INT)
BEGIN
    UPDATE `categories` SET `deleted` = 1, `lastmodifiedon` = NOW(), `lastmodifiedby` = p_userid
    WHERE clientid = p_clientid AND `categoryid` = p_id;
END $$

DROP PROCEDURE IF EXISTS `spdeletecustomer` $$
CREATE PROCEDURE `spdeletecustomer`(IN clientid INT, IN p_customerid INT, IN p_userid INT)
BEGIN
    UPDATE `customers` SET deleted = 1, lastmodifiedon = NOW(), lastmodifiedby = p_userid
    WHERE clientid = p_clientid AND customerid = p_customerid;
END $$
...
CREATE PROCEDURE `spdeleteglaccount`(`$id` INT, `$userid` INT)
BEGIN
...
CREATE PROCEDURE `spdeleteglgroup`(`$id` INT, `$userid` INT)
BEGIN
...
CREATE PROCEDURE `spdeletepos`(`$posid` NUMERIC, `$userid` NUMERIC)
BEGIN
...
CREATE PROCEDURE `spdeleteproduct`(IN clientid INT, IN p_productid INT, IN p_userid INT)
BEGIN
    UPDATE `products` SET `deleted` = 1, `lastmodifiedon` = NOW(), `lastmodifiedby` = p_userid 
    WHERE clientid = p_clientid AND `productid` = p_productid;
END $$

DROP PROCEDURE IF EXISTS `spdeleterole` $$
CREATE PROCEDURE `spdeleterole`(`$roleid` INT, `$userid` INT)
BEGIN
	UPDATE `roles` 
...
CREATE PROCEDURE `spdeletesupplier`(`$supplierid` NUMERIC, `$userid` NUMERIC)
BEGIN
...
CREATE PROCEDURE `spdeleteuser`(`$id` INT, `$userid` INT)
BEGIN
...
CREATE PROCEDURE `spdisableuseraccount`(`$id` INT, `$reason` VARCHAR(500), `$userid` INT)
BEGIN
...
CREATE PROCEDURE `spenableuseraccount`(`$id` INT, `$userid` INT)
BEGIN
...
CREATE PROCEDURE `spfilterproductsbyname`(IN clientid INT, IN p_name VARCHAR(50), IN p_posid INT)
BEGIN
    SELECT * FROM `products` 
    WHERE clientid = p_clientid AND `itemname` LIKE CONCAT('%', p_name, '%') AND `deleted` = 0 
...
CREATE PROCEDURE `spgetbestsellingcategory`(IN p_branchid INT, IN $startdate DATETIME, IN $enddate DATETIME)
BEGIN
	...
	AND o.branchid = p_branchid AND DATE_FORMAT(`receiptdate`,'%Y-%m-%d') BETWEEN $startdate AND $enddate
...
CREATE PROCEDURE `spgetbestsellingproducts`(IN p_branchid INT, IN $startdate DATETIME, IN $enddate DATETIME, IN $userid INT)
BEGIN
	IF $userid=0 THEN
	...
	ELSE
		...
		FROM `categories` c,`products` p,`possales` o,`possalesdetails` s
		WHERE c.`categoryid`=p.`categoryid` AND p.`productid`=s.`itemcode` AND s.`possaleid`=o.possaleid
		AND o.branchid = p_branchid AND DATE_FORMAT(`receiptdate`,'%Y-%m-%d') BETWEEN $startdate AND $enddate AND o.addedby=$userid
		GROUP BY `categoryname`,`itemname`
		ORDER BY SUM(quantity) DESC 
		LIMIT 5;
	END IF;
...
BEGIN
    SELECT * FROM `categories` WHERE clientid = p_clientid AND `deleted` = 0 ORDER BY `categoryname`;
END $$

DROP PROCEDURE IF EXISTS `spgetcategorydetails` $$
CREATE PROCEDURE `spgetcategorydetails`(IN clientid INT, IN p_id INT)
BEGIN
    SELECT * FROM `categories` WHERE clientid = p_clientid AND `categoryid` = p_id;
END $$
...
BEGIN
    SELECT * FROM `customercategories` WHERE clientid = p_clientid AND `deleted` = 0 ORDER BY `categoryname`;
END $$
...
CREATE PROCEDURE `spgetcustomerdetails`(IN clientid INT, IN p_customerid INT)
BEGIN
    SELECT * FROM `customers` WHERE clientid = p_clientid AND customerid = p_customerid;
END $$
...
CREATE PROCEDURE `spgetcustomers`(
    IN clientid INT,
    IN p_posid INT,
    IN p_regularcustomers TINYINT,
    IN p_onetimecustomers TINYINT
)
BEGIN
    SELECT * FROM `customers` 
    WHERE clientid = p_clientid AND `deleted` = 0
    AND ((p_regularcustomers = 1 AND onetimecustomer = 0) OR (p_onetimecustomers = 1 AND onetimecustomer = 1))
...
CREATE PROCEDURE `spgetdashboardheaders`(IN p_branchid INT, IN $date DATETIME)
BEGIN
	SELECT 
	(SELECT SUM(amount) FROM `vwsalessummary2` WHERE branchid = p_branchid AND DATE_FORMAT(transactiondate,'%Y-%m-%d') = $date) AS today_sales,
	(SELECT COUNT(DISTINCT receiptno) FROM `vwsalessummary2` WHERE branchid = p_branchid AND DATE_FORMAT(transactiondate,'%Y-%m-%d') = $date) AS today_customers,
	(SELECT SUM(amount) FROM `vwsalessummary2` WHERE branchid = p_branchid AND DATE_FORMAT(transactiondate,'%Y-%m') = DATE_FORMAT($date,'%Y-%m')) AS month_sales,
	(SELECT COUNT(DISTINCT receiptno) FROM `vwsalessummary2` WHERE branchid = p_branchid AND DATE_FORMAT(transactiondate,'%Y-%m') = DATE_FORMAT($date,'%Y-%m')) AS month_customers;
END $$
...
BEGIN
    SELECT * FROM `glaccounts` WHERE clientid = p_clientid AND `deleted` = 0 ORDER BY `accountname`;
END $$
...
BEGIN
    SELECT * FROM institution WHERE clientid = p_clientid;
END $$
...
CREATE PROCEDURE `spgetnonuserroles`(`$userid` INT)
BEGIN
	SELECT * FROM `roles` 
...
CREATE PROCEDURE `spgetobjects`(IN clientid INT, IN p_module VARCHAR(50))
BEGIN
    IF p_module='' OR p_module IS NULL THEN
        SELECT `id`,`description`,`module` FROM `objects` WHERE clientid = p_clientid ORDER BY `description`;
    ELSE
        SELECT `id`,`description`,`module` FROM `objects` WHERE clientid = p_clientid AND `module` = p_module ORDER BY `description`;
    END IF;
END $$
...
CREATE PROCEDURE `spgetpaymentmethods`(IN p_branchid INT)
BEGIN
    SELECT * FROM paymentmethods WHERE branchid = p_branchid;
END $$
...
BEGIN
    SELECT * FROM `pointsofsale` WHERE clientid = p_clientid AND `deleted` = 0 ORDER BY `description`;
END $$
...
BEGIN
    SELECT * FROM `products` WHERE clientid = p_clientid AND `categoryid` = p_categoryid AND `deleted` = 0 ORDER BY `itemname`;
END $$

DROP PROCEDURE IF EXISTS `spgetproductdetails` $$
CREATE PROCEDURE `spgetproductdetails`(IN clientid INT, IN p_productcode VARCHAR(50), IN p_customerid INT, IN p_storeid INT)
BEGIN
    SELECT * FROM `products` WHERE clientid = p_clientid AND (`itemcode` = p_productcode OR `itemname` = p_productcode) AND `deleted` = 0;
END $$
...
CREATE PROCEDURE `spgetreceiptdetails`(IN p_branchid INT, IN p_receiptno VARCHAR(50))
BEGIN
    ...
    WHERE s.branchid = p_branchid AND s.receiptno = p_receiptno;
END $$
...
BEGIN
    SELECT * FROM `roles` WHERE clientid = p_clientid AND `deleted` = 0 ORDER BY `rolename`;
END $$
...
CREATE PROCEDURE `spgetsalesbypaymentmode`(IN p_branchid INT, IN $startdate DATETIME, IN $enddate DATETIME, IN $userid INT)
BEGIN
	IF $userid=0 THEN 
		SELECT paymentmode AS `paymentmode`,SUM(receipttotal) AS amount
		FROM `vwsalessummary2` 
		WHERE branchid = p_branchid AND DATE_FORMAT(transactiondate,'%Y-%m-%d') BETWEEN $startdate AND $enddate
		GROUP BY paymentmode 
		ORDER BY transactiondate;
	ELSE
		SELECT paymentmode AS `paymentmode`,SUM(receipttotal) AS amount
...
CREATE PROCEDURE `spgetsalesbyquantity`(IN p_branchid INT, IN $startdate DATETIME, IN $enddate DATETIME, IN $daterange VARCHAR(50), IN $userid INT)
BEGIN
	IF $userid=0 THEN
		IF $daterange='Day' THEN
			SELECT DATE_FORMAT(transactiondate,'%H') AS `transactiondate`,SUM(quantity) AS quantity
			FROM `vwsalessummary2` v WHERE branchid = p_branchid AND DATE_FORMAT(`transactiondate`,'%Y-%m-%d') BETWEEN $startdate AND $enddate
			GROUP BY DATE_FORMAT(transactiondate,'%H') 
			ORDER BY v.transactiondate;
		ELSEIF $daterange='Week' THEN
...
CREATE PROCEDURE `spgetsalessettings`(IN p_branchid INT)
BEGIN
    SELECT * FROM salessettings WHERE branchid = p_branchid;
END $$
...
CREATE PROCEDURE `spgetsalestrend`(IN p_branchid INT, IN $startdate DATETIME, IN $enddate DATETIME, IN $daterange VARCHAR(50), IN $userid INT)
BEGIN
	IF $userid=0 THEN 
		IF $daterange='Day' THEN
...
BEGIN
    SELECT * FROM `suppliers` WHERE clientid = p_clientid AND `deleted` = 0 ORDER BY `suppliername`;
END $$
...
BEGIN
    ...
    WHERE clientid = p_clientid AND `module` IS NOT NULL 
    ORDER BY `module`;
END $$
...
BEGIN
    SELECT * FROM `taxtypes` WHERE clientid = p_clientid;
END $$

DROP PROCEDURE IF EXISTS `spgettodaysdate` $$
CREATE PROCEDURE `spgettodaysdate`(IN p_branchid INT)
BEGIN
...
CREATE PROCEDURE `spgetuserdetails`(IN p_username VARCHAR(50))
BEGIN
    SELECT * FROM user WHERE username = p_username;
END $$

DROP PROCEDURE IF EXISTS `spgetusernamefromuserid` $$
CREATE PROCEDURE `spgetusernamefromuserid`(`$userid` INT)
BEGIN
	SELECT * FROM `user` WHERE `id`=$userid;
    END $$

DROP PROCEDURE IF EXISTS `spgetuseroutlets` $$
CREATE PROCEDURE `spgetuseroutlets`(`$userid` INT)
BEGIN
	SELECT o.*,`posname`
...
CREATE PROCEDURE `spgetuserroles`(`$userid` INT)
BEGIN
...
BEGIN
    SELECT * FROM `warehouses` WHERE clientid = p_clientid ORDER BY `description`;
END $$

DROP PROCEDURE IF EXISTS `spgteunitsofmeasure` $$
CREATE PROCEDURE `spgteunitsofmeasure`(IN clientid INT)
BEGIN
    SELECT * FROM `unitsofmeasure` WHERE clientid = p_clientid ORDER BY `description`;
END $$
...
CREATE PROCEDURE `spsaceglgroupname`(`$id` INT, `$glaccountclass` INT, `$groupname` VARCHAR(50), `$subcategoryof` INT, `$cashbookaccount` INT, `$userid` INT)
BEGIN
	IF $id=0 THEN
...
CREATE PROCEDURE `spsavecategory`(
    IN clientid INT,
    IN p_id INT,
...
BEGIN
    IF p_id = 0 THEN
        ...
        VALUES (clientid, p_name, p_prefix, p_currentno, NOW(), p_userid, 0);
    ELSE
        UPDATE `categories` SET 
            `categoryname` = p_name, `prefix` = p_prefix, `currentno` = p_currentno, 
            `lastmodifiedon` = NOW(), `lastmodifiedby` = p_userid
        WHERE clientid = p_clientid AND `categoryid` = p_id;
    END IF;
END $$
...
CREATE PROCEDURE `spsavecustomer`(
    ...
    IN p_tradingname VARCHAR(100),
    IN p_physicaladdress VARCHAR(100),
    IN p_postaladdress VARCHAR(100),
    ...
    IN p_category INT,
    IN p_posid INT,
    IN p_onetimecustomer TINYINT,
    IN p_pinno VARCHAR(50),
    IN p_idno VARCHAR(50),
    IN p_subzoneid INT
)
BEGIN
    IF p_customerid = 0 THEN
        INSERT INTO `customers` (
            ...
            clientid, p_customername, p_tradingname, p_physicaladdress, p_postaladdress, p_mobile, p_email,
            p_creditlimit, p_creditterm, NOW(), p_userid, 0, p_category, p_posid,
            p_onetimecustomer, p_pinno, p_idno, p_subzoneid
        );
    ELSE
        UPDATE `customers` SET
            customername = p_customername,
            tradingname = p_tradingname,
            physicaladdress = p_physicaladdress,
            postaladdress = p_postaladdress,
            mobile = p_mobile,
            email = p_email,
            creditlimit = p_creditlimit,
            creditterm = p_creditterm,
            lastmodifiedon = NOW(),
            lastmodifiedby = p_userid,
            categoryid = p_category,
            pointofsaleid = p_posid,
            onetimecustomer = p_onetimecustomer,
            pinno = p_pinno,
            idno = p_idno,
            subzoneid = p_subzoneid
        WHERE clientid = p_clientid AND customerid = p_customerid;
    END IF;
END $$
...
CREATE PROCEDURE `spsaveglaccount`(`$id` INT, `$groupid` INT, `$accountcode` VARCHAR(50), `$accountname` VARCHAR(50), `$userid` INT)
BEGIN
	IF $id=0 THEN 
...
CREATE PROCEDURE `spsaveglgroupname`(`$id` INT, `$glaccountclass` INT, `$groupname` VARCHAR(50), `$subcategoryof` INT, `$cashbookaccount` INT, `$userid` INT)
BEGIN
	IF $id=0 THEN
		INSERT INTO `glaccountgroups`(`groupname`,`subactegoryof`,`dateadded`,`addedby`,`deleted`,`cashbookaccount`,`glaccountclass`)
		VALUES($groupname,$subcategoryof,NOW(),$userid,0,$cashbookaccount,$glaccountclass);
...
CREATE PROCEDURE `spsavegoodsreceived`(
    ...
    IN p_warehouseid NUMERIC,
    IN p_supplierid NUMERIC,
    IN p_deliverynoteno VARCHAR(50),
    IN p_userid NUMERIC,
    IN p_saveinvoice BOOLEAN,
    IN p_invoiceno VARCHAR(50),
    IN p_inspectedby INT,
    IN p_transferitems INT,
    IN p_transferpos INT
)
BEGIN
    DECLARE v_grnno VARCHAR(50);
    START TRANSACTION;
        SET v_grnno = fngenerategrnno(p_branchid);

        INSERT INTO `goodsreceived`(branchid, `warehouseid`, `grnno`, `datereceived`, `supplierid`, `deliverynono`, `receivedby`, `status`, `inspectedby`)
        VALUES(p_branchid, p_warehouseid, v_grnno, NOW(), p_supplierid, p_deliverynoteno, p_userid, 'Confirmed', p_inspectedby);

        INSERT INTO `goodsreceiveddetails`(branchid, `grnno`, `itemcode`, `purchaseorderno`, `quantity`, `serialno`)
        SELECT p_branchid, v_grnno, `itemcode`, `purchaseorderno`, `quantity`, `serialno` FROM `tempgoodsreceived` WHERE branchid = p_branchid AND `refno` = p_refno;
        ...
        SELECT p_branchid, DATE_FORMAT(NOW(),'%Y-%m-%d'), tg.`itemcode`, p.id, tg.`quantity`,
            CASE WHEN `taxinclusive` = 1 THEN (100 / (100 + p.taxrate)) * pd.unitprice ELSE pd.unitprice END,
            p.taxrate, p.taxid
        FROM `tempgoodsreceived` tg 
        JOIN `purchaseorders` p ON p.branchid = p_branchid AND p.`purchaseorderno` = tg.`purchaseorderno`
        JOIN `purchaseorderdetails` pd ON pd.branchid = p_branchid AND pd.`purchaseorderid` = p.`id`
        WHERE tg.branchid = p_branchid AND tg.`refno` = p_refno AND pd.itemcode = tg.itemcode;
        
        
        UPDATE `serials` SET `currentno` = `currentno` + 1 WHERE branchid = p_branchid AND `documenttype` = 'Goods Received Note';
        
        
        DELETE FROM `tempgoodsreceived` WHERE branchid = p_branchid AND `refno` = p_refno;
...
CREATE PROCEDURE `spsavepos`(`$id` NUMERIC, `$posname` VARCHAR(50), `$postype` VARCHAR(50), `$printkitchenorders` BOOL, `$userid` NUMERIC)
BEGIN
	IF $id=0 THEN 
...
CREATE PROCEDURE `spsavepossale`(
    IN p_branchid INT,
    IN p_refno VARCHAR(50),
    IN p_customerid INT,
    IN p_posid INT,
...
BEGIN
    ...
    DECLARE cur_products CURSOR FOR 
        ...
        FROM tempsale WHERE branchid = p_branchid AND refno = p_refno;
    
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET v_finished = 1;

    START TRANSACTION;
        
        SET v_receiptno = (SELECT spgeneratecustomerreceiptno(p_branchid));
        SET v_customername = (SELECT customername FROM customers WHERE branchid = p_branchid AND customerid = p_customerid);

        
        INSERT INTO possales (branchid, receiptno, customerid, pointofsaleid, receiptdate, reference, addedby, deleted)
        VALUES (p_branchid, v_receiptno, p_customerid, p_posid, p_transactiondate, p_reference, p_userid, 0);
        
        SET v_id = LAST_INSERT_ID();

        
        INSERT INTO possalesdetails (branchid, possaleid, itemcode, quantity, unitprice, discount, serialno, description)
        SELECT p_branchid, v_id, itemcode, quantity, unitprice, discount, serialno, description
        FROM tempsale WHERE branchid = p_branchid AND refno = p_refno;

        
        INSERT INTO possalespayments (branchid, possaleid, paymentmode, reference, amount)
        SELECT p_branchid, v_id, paymentmode, reference, amount
        FROM temppossalespayment WHERE branchid = p_branchid AND refno = p_refno;

        
        UPDATE serials SET currentno = currentno + 1 WHERE branchid = p_branchid AND documenttype = 'Customer Receipt';
        ...
        loop_products: LOOP
            ...
            WHILE v_tempquantity > 0 DO
                ...
                WHERE s.branchid = p_branchid AND s.productid = v_tempproductid
                ...
                ELSE
                    ...
                    VALUES (p_branchid, v_stockmovementid, v_id, v_fifoquantity, v_tempunitprice - v_tempdiscount);
                END IF;
            END WHILE;
        ...
        DELETE FROM tempsale WHERE branchid = p_branchid AND refno = p_refno;
        DELETE FROM temppossalespayment WHERE branchid = p_branchid AND refno = p_refno;
...
CREATE PROCEDURE `spsaveprivileges`(`$id` INT, `$category` VARCHAR(50), `$refno` VARCHAR(50), `$userid` INT)
BEGIN
	START TRANSACTION;
...
CREATE PROCEDURE `spsaveproduct`(
    ...
    IN p_categoryid NUMERIC,
    IN p_uom VARCHAR(50),
    IN p_buyingprice DECIMAL(18,2),
    IN p_sellingprice DECIMAL(18,2),
    IN p_reorderlevel NUMERIC,
    IN p_userid NUMERIC,
    IN p_refno VARCHAR(50),
    IN p_generateitemcode BOOL,
    IN p_canserialize BOOL,
    IN p_bundleitem BOOLEAN,
    IN p_taxtypeid INT,
    IN p_itemlength DECIMAL(18,2),
    IN p_itemwidth DECIMAL(18,2),
    IN p_itemheight DECIMAL(18,2),
    IN p_allownegativesales BOOL,
    IN p_saleby VARCHAR(50),
    IN p_bundleproduct INT,
    IN p_allowreturnexchange BOOL
)
BEGIN
    IF p_id=0 THEN 
        IF p_generateitemcode=1 THEN
            SET p_itemcode=(SELECT fngenerateproductcode(clientid, p_categoryid));
            UPDATE `categories` SET `currentno`=`currentno`+1 WHERE clientid = p_clientid AND `categoryid`=p_categoryid;
        END IF;
        INSERT INTO `products`(
            ...
            clientid, p_itemcode, p_itemname, p_uom, p_buyingprice, p_sellingprice, p_categoryid, NOW(), p_userid, 0, p_reorderlevel, p_canserialize,
            p_bundleitem, p_taxtypeid, p_itemlength, p_itemwidth, p_itemheight, p_allownegativesales, p_saleby, p_bundleproduct, p_allowreturnexchange
        );
    ELSE
        UPDATE `products` SET 
            `itemcode`=p_itemcode, `itemname`=p_itemname, `unitofmeasure`=p_uom, `buyingprice`=p_buyingprice, `sellingprice`=p_sellingprice,
            `categoryid`=p_categoryid, `reorderlevel`=p_reorderlevel, `lastmodifiedon`=NOW(), `lastmodifiedby`=p_userid, `serializable`=p_canserialize, 
            `bundleitem`=p_bundleitem, `taxtypeid`=p_taxtypeid, `length`=p_itemlength, `width`=p_itemwidth, `height`=p_itemheight, `allownegativesales`=p_allownegativesales,
            `saleby`=p_saleby, `bundledproduct`=p_bundleproduct, `allowreturnexchange`=p_allowreturnexchange
        WHERE clientid = p_clientid AND `productid`=p_id;
    END IF;
END $$

DROP PROCEDURE IF EXISTS `spsavepurchaseorder` $$
CREATE PROCEDURE `spsavepurchaseorder`(
    IN p_branchid INT,
    IN p_id NUMERIC,
    IN p_refno VARCHAR(50),
    IN p_supplierid INT,
    ...
    IN p_currencyid INT,
    IN p_exchangerate DECIMAL(18,2),
    IN p_departmentid INT,
    IN p_taxid INT,
    IN p_taxrate DECIMAL(18,2),
    IN p_userid INT
)
BEGIN
    ...
    IF p_id=0 THEN 
        START TRANSACTION;
            SET v_purchaseorderno = fngeneratepurchaseorderno(p_branchid);
            ...
            INSERT INTO `purchaseorders`(
                ...
                p_branchid, v_purchaseorderno, NOW(), p_supplierid, NOW(), 'Pending', p_terms, p_departmentid, p_category,
                p_taxid, p_exchangerate, p_currencyid, p_taxrate, p_userid
            );
        
            SET v_orderid = LAST_INSERT_ID();
        
            INSERT INTO `purchaseorderdetails`(branchid, `purchaseorderid`, `itemcode`, `quanity`, `unitprice`, `taxable`, `taxinclusive`)
            SELECT p_branchid, v_orderid, `itemcode`, `quantity`, `unitprice`, `taxable`, `taxinclusive` 
            FROM `temppurchaseorder` WHERE branchid = p_branchid AND `refno` = p_refno;
        
            DELETE FROM `temppurchaseorder` WHERE branchid = p_branchid AND `refno` = p_refno;
            
            UPDATE `serials` SET `currentno` = `currentno` + 1 WHERE branchid = p_branchid AND `documenttype` = 'Purchase Order';
        COMMIT;
    ELSE
        START TRANSACTION;
            DELETE FROM `purchaseorderdetails` WHERE branchid = p_branchid AND `purchaseorderid` = p_id;
            
            UPDATE `purchaseorders` SET 
                `supplierid` = p_supplierid, `terms` = p_terms, `departmentid` = p_departmentid, `category` = p_category,
                `currencyid` = p_currencyid, `exchangerate` = p_exchangerate, `lastmodifiedon` = NOW(), `lastmodifiedby` = p_userid, `taxid` = p_taxid,
                `taxrate` = p_taxrate
            WHERE branchid = p_branchid AND `id` = p_id;
            
            INSERT INTO `purchaseorderdetails`(branchid, `purchaseorderid`, `itemcode`, `quanity`, `unitprice`, `taxable`, `taxinclusive`)
            SELECT p_branchid, p_id, `itemcode`, `quantity`, `unitprice`, `taxable`, `taxinclusive` 
            FROM `temppurchaseorder` WHERE branchid = p_branchid AND `refno` = p_refno;
            
            DELETE FROM `temppurchaseorder` WHERE branchid = p_branchid AND `refno` = p_refno;
            
            SET v_purchaseorderno = (SELECT `purchaseorderno` FROM `purchaseorders` WHERE branchid = p_branchid AND `id` = p_id);
        COMMIT;
    END IF;
...
CREATE PROCEDURE `spsaverole`(`$roleid` INT, `$rolename` VARCHAR(50), `$roledescription` VARCHAR(50), `$userid` INT)
BEGIN
	IF $roleid=0 THEN
...
CREATE PROCEDURE `spsavesupplier`(`$supplierid` INT, `$suppliername` VARCHAR(50), `$physicaladdress` VARCHAR(100), `$postaladdress` VARCHAR(100), `$creditlimit` NUMERIC, `$mobile` VARCHAR(50), `$email` VARCHAR(50), `$supplierpinno` VARCHAR(50), `$userid` NUMERIC)
BEGIN
	IF $supplierid=0 THEN 
...
CREATE PROCEDURE `spsavetempgoodsreceived`(
    IN p_branchid INT,
    IN p_refno VARCHAR(50),
...
BEGIN
    ...
    VALUES (p_branchid, p_refno, p_pono, p_itemcode, p_quantity, p_serialno);
END $$
...
CREATE PROCEDURE `spsavetemppurchaseorderitem`(
    IN p_branchid INT,
    IN p_refno VARCHAR(50),
    IN p_itemcode VARCHAR(50),
    IN p_quantity DECIMAL(18,2),
    ...
    IN p_taxable BOOL,
    IN p_taxinclusive BOOL
)
BEGIN
    INSERT INTO `temppurchaseorder` (branchid, `refno`, `itemcode`, `quantity`, `unitprice`, `taxable`, `taxinclusive`)
    VALUES (p_branchid, p_refno, p_itemcode, p_quantity, p_unitprice, p_taxable, p_taxinclusive);
END $$
...
CREATE PROCEDURE `spsavetempsale`(
    IN p_branchid INT,
    IN p_refno VARCHAR(50),
    IN p_itemcode VARCHAR(50),
    IN p_quantity DECIMAL(18,2),
...
BEGIN
    ...
    VALUES (p_branchid, p_refno, p_itemcode, p_quantity, p_unitprice, p_discount, p_serialno, p_description);
END $$
...
CREATE PROCEDURE `spsaveuser`(`$userid` INT, `$userpassword` VARCHAR(50), `$systemadmin` BIT, `$username` VARCHAR(50), `$firstname` VARCHAR(50), `$middlename` VARCHAR(50), `$lastname` VARCHAR(50), `$email` VARCHAR(50), `$mobile` VARCHAR(50), `$changepasswordonlogon` BIT, `$accountactive` BIT, `$addedby` INT)
BEGIN
	IF $userid=0 THEN 
		-- begin
...
CREATE PROCEDURE `spvalidateuserprivilege`(`$userid` INT, `$objectid` INT)
BEGIN
	DECLARE $admin INT;
...
CREATE PROCEDURE `sp_checkuserprivilegewithcode`(`$userid` INT, `$objectcode` VARCHAR(50))
BEGIN
...
CREATE PROCEDURE `sp_deletecustomercontact`(`$id` INT, `$userid` INT)
BEGIN
...
CREATE PROCEDURE `sp_deleterawmaterial`(`$materialid` INT, `$userid` INT)
BEGIN
	UPDATE `rawmaterials` 
	SET `deleted`=1,`datedeleted`=NOW(),`deletedby`=$userid
...
CREATE PROCEDURE `sp_deletezone`(`$id` INT, `$userid` INT)
BEGIN
...
BEGIN
    SELECT * FROM `currencies` WHERE clientid = p_clientid;
END $$
...
CREATE PROCEDURE `sp_getdefaultterms`(IN p_branchid INT)
BEGIN
    SELECT * FROM `defaultterms` WHERE branchid = p_branchid;
END $$
...
BEGIN
    SELECT * FROM `departments` WHERE clientid = p_clientid AND deleted = 0;
END $$
...
CREATE PROCEDURE `sp_getpapergrammage`(IN p_branchid INT)
BEGIN
    SELECT * FROM `papergrammage` WHERE branchid = p_branchid;
END $$
...
CREATE PROCEDURE `sp_getsubzones`(`$parent` INT)
BEGIN
	IF $parent=0 THEN
		SELECT z.id,z.zonename,z.parent,z.dateadded,COUNT(c.customerid) customers, CONCAT(`firstname`,' ',`middlename`,' ',`lastname`) addedbyname 
		FROM `zones` z
...
CREATE PROCEDURE `sp_gettaxdetails`(IN clientid INT, IN p_taxid INT)
BEGIN
    SELECT * FROM `taxtypes` WHERE clientid = p_clientid AND id = p_taxid;
END $$
...
CREATE PROCEDURE `sp_resetuserpin`(`$userid` INT, `$pin` VARCHAR(100), `$salt` VARCHAR(50))
BEGIN
		UPDATE `user`
...
CREATE PROCEDURE `sp_savecustomercontact`(`$id` INT, `$customerid` INT, `$categoryid` INT, `$contactname` VARCHAR(100), `$mobile` VARCHAR(100), `$email` VARCHAR(100), `$userid` INT)
BEGIN
	IF $id=0 THEN 
...
CREATE PROCEDURE `sp_savedepartment`(`$id` INT, `$departmentname` VARCHAR(50), `$userid` INT, `$hodid` INT)
BEGIN
	IF $id=0 THEN 
...
CREATE PROCEDURE `sp_savefleetvehicle`(`$vehicleid` INT, `$regno` VARCHAR(50), `$bodytypeid` INT, `$fueltypeid` INT, `$enginerating` INT, `$userid` INT)
BEGIN
	IF $vehicleid=0 THEN
...
CREATE PROCEDURE `sp_saveinstitutiondetails`(
    ...
    IN p_physicaladdress VARCHAR(100),
    IN p_postaladdress VARCHAR(100),
    IN p_landline VARCHAR(50),
    IN p_email VARCHAR(50),
    IN p_mobile VARCHAR(50),
    IN p_pinno VARCHAR(50),
    IN p_autoinvoicegrn TINYINT,
    IN p_postalcode VARCHAR(50),
    IN p_tagline VARCHAR(1000),
    IN p_website VARCHAR(100),
    IN p_receiptfooter VARCHAR(4000),
    IN p_defaultcustomer INT,
    IN p_mainbusinesstype VARCHAR(50),
...
BEGIN
    IF EXISTS (SELECT * FROM institution WHERE clientid = clientid) THEN
        UPDATE institution SET
            ...
            mobile = p_mobile,
            pinno = p_pinno,
            autoaddinvoiceduringgrn = p_autoinvoicegrn,
            postalcode = p_postalcode,
            tagline = p_tagline,
            website = p_website,
            receiptfooter = p_receiptfooter,
            defaultcustomerid = p_defaultcustomer,
            mainbusinesstype = p_mainbusinesstype,
            logo = p_logo,
            town = p_town
    ...
    ELSE
        INSERT INTO institution (
            ...
            clientid, p_companyname, p_physicaladdress, p_postaladdress, p_landline, p_email, p_mobile, p_pinno,
            p_autoinvoicegrn, p_postalcode, p_tagline, p_website, p_receiptfooter,
            p_defaultcustomer, p_mainbusinesstype, p_logo, p_town
        );
    END IF;
END $$
...
CREATE PROCEDURE `sp_saverawmaterial`(`$materialid` INT, `$categoryid` INT, `$materialname` VARCHAR(50), `$uom` VARCHAR(50), `$physicalproduct` BOOL, `$unitprice` DECIMAL(18,2), `$itemcode` VARCHAR(50), `$generateitemcode` BOOL, `$userid` INT)
BEGIN
...
CREATE PROCEDURE `sp_saveuser`(
    ...
    IN p_mobile VARCHAR(50),
    IN p_changepasswordonlogon TINYINT,
    IN p_accountactive TINYINT,
    IN p_pin VARCHAR(100),
    IN p_pinsalt VARCHAR(50),
    IN p_addedby INT
)
BEGIN
    IF p_userid = 0 THEN 
        INSERT INTO `user`(
            ...
            clientid, p_username, p_userpassword, p_salt, p_firstname, p_middlename, p_lastname, p_email, p_mobile, 
            p_changepasswordonlogon, p_accountactive, p_addedby, NOW(), p_systemadmin, p_pin, p_pinsalt
        );
    ...
    ELSE
        UPDATE `user` SET 
            ...
            mobile = p_mobile,
            changepasswordonlogon = p_changepasswordonlogon, 
            accountactive = p_accountactive,
            systemadmin = p_systemadmin, 
            lastmodifiedby = p_addedby, 
            lastmodifiedon = NOW(),
            pin = p_pin,
            pinsalt = p_pinsalt
        WHERE clientid = p_clientid AND userid = p_userid;
...
CREATE PROCEDURE `sp_saveuserprofilephoto`(`$userid` INT, `$documentname` VARCHAR(1000))
BEGIN
	UPDATE `user` SET `profilephoto`=$documentname WHERE `id`=$userid;
    END $$

DROP PROCEDURE IF EXISTS `sp_saveusersignature` $$
CREATE PROCEDURE `sp_saveusersignature`(`$userid` INT, `$documentname` VARCHAR(1000))
BEGIN
	UPDATE `user` SET `signature`=$documentname WHERE `id`=$userid;
    END $$

DROP PROCEDURE IF EXISTS `sp_savezone` $$
CREATE PROCEDURE `sp_savezone`(`$id` INT, `$zonename` VARCHAR(50), `$parent` INT, `$userid` INT)
BEGIN
	IF $id=0 THEN
...
CREATE PROCEDURE `sp_subzones`(`$parent` INT)
BEGIN
	IF $parent=0 THEN
		SELECT z.id,z.zonename,z.parent,z.dateadded,COUNT(c.customerid) customers, CONCAT(`firstname`,' ',`middlename`,' ',`lastname`) addedbyname 
		FROM `zones` z
...
CREATE PROCEDURE `sp_validatepurchaseorderapproval`(`$userid` INT, `$approvallevel` INT, `$departmentid` INT)
BEGIN
...
CREATE PROCEDURE `sp_validaterequisitionapproval`(`$userid` INT, `$approvallevel` INT, `$departmentid` INT)
BEGIN
	DECLARE $admin INT DEFAULT 0;
	DECLARE $valid INT DEFAULT 0;
	SET $admin=(SELECT systemadmin FROM `user` WHERE `id`=$userid);
