        ...
        <table width="100%" class="header-table" cellspacing="0">
            <tr>
                ...
                <td width="30%" style="text-align: right;">
                    <!-- Order No.<br /><span style="font-weight: bold; font-size: 12pt;">'.$pono.'</span> -->
                    <br/>
...
// document heading
$data.="<h3 style='text-align:center; padding-top:100px;' >PURCHASE ORDER</h3>";
...
$data.="<td width='50%' style='text-align:right;'> Order Number: <strong>".$pono."</strong><br/>Order Date: ". $rst2['orderdate']."<br />Expected Date: ". $rst2['expecteddate']." <br/>Currency:". $rst2['currencyname']."</td></tr></table>";
...
$data.="<table  width='100%' cellpadding='3' cellspacing='0' class='items'><thead><tr><td style='text-align:left;' class='border-bottom'>Item Code</td><td style='text-align:left;' class='border-bottom'>Item Name</td><td style='text-align:left;' class='border-bottom'>UoM</td><td style='text-align:right;' class='border-bottom'>Quantity</td><td style='text-align:right;' class='border-bottom'>Unit Price</td><td style='text-align:right;' class='border-bottom'>Total</td></tr></thead><tbody>";
...
while($row=$rst->fetch()){
    $taxname=$row['taxname'];
    $unitprice=$row['taxinclusive']==0?$row['unitprice']:(100/(100+$row['taxrate']))*$row['unitprice'];
    $data.="<tr ><td width='12.5%' class='border-bottom'>".$row['itemcode']."</td>";
    $data.="<td width='40%' class='border-bottom'>".$row['itemname']."</td>";
    ...
    $data.="<td  width='12.5%' style='text-align:right' class='border-bottom'>".number_format($unitprice,2)."</td>";
    $data.="<td  width='15%' style='text-align:right' class='border-bottom'>".number_format($unitprice*$row['quantity'],2)."</td></tr>";
    ...
    // if($taxrate==1){
        if($taxinclusive==1){
            $taxes+=$row['total']*$taxrate/(100+$taxrate);
        }else{
            $taxes+=$row['total']*$taxrate/100;
        }
    // }
}
// add summaries for the payments 
$sumtotal=$total+$taxes;
$data.="<tr class='totals'><td colspan='5' style='text-align:right'>TOTAL :</td><td style='text-align:right'>". number_format($total,2) ."</td></tr>";
$data.="<tr><td colspan='5' style='text-align:right'>".$taxname." :</td><td style='text-align:right'>". number_format($taxes,2) ."</td></tr>";
$data.="<tr><td colspan='5' style='text-align:right'>SUM TOTAL:</td><td style='text-align:right'>". number_format($sumtotal,2) ."</td></tr>";
$data.="</tbody></table>";
...
// add signatories
$sql="CALL sp_getpurchaseorderapprovers('{$pono}')";
$rst=$db->getData($sql);
...
$documentname= preg_replace('/\s+/', '_', $suppliername)."_".$pono.".pdf";
