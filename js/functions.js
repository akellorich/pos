class ESCPOSEncoder {
  ...
  cut() {
    ...
    this.commands.push('\n\n'); // Feed 2 lines (to flush the buffer)
    return this;
  }
...
function json2table(tablename, data, pagelength = 15) {
  if (data.length > 0) {
    ...
    tablename.DataTable({
      dom: '<"dt-buttons-container mb-3"B><"dt-controls-container d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-3"lf>rtip', // Adds buttons to the interface B- Buttons l - Length f-Filter,f-Search
...
async function connecttoprinter() {
  // Dynamically determine the server root URL
  const getBaseURL = () => {
    const pathname = window.location.pathname;
    const posIndex = pathname.indexOf('/pos/');
    if (posIndex !== -1) {
  ...
  qz.security.setCertificatePromise(function (resolve, reject) {
    // Dynamic fetch from the local machine
    fetch("http://localhost/qztray/digital-certificate.php", { cache: 'no-store', headers: { 'Content-Type': 'text/plain' } })
      .then(function (data) {
