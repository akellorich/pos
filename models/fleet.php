<?php
    ...
    class fleet extends db{
        ...
        public function checkvehicleregno($id,$regno){
            $sql="CALL `sp_checkfleetvehicleregno`({$this->clientid},$id,'{$regno}')";
        ...
        public function savevehicle($vehicleid,$regno,$bodytypeid, $fueltypeid, $enginerating){
            if(!$this->checkvehicleregno($vehicleid,$regno)){
                $sql="CALL `sp_savefleetvehicle`({$this->clientid},{$vehicleid},'{$regno}',{$bodytypeid},{$fueltypeid},{$enginerating},{$_SESSION['userid']})";
        ...
        public function savefuelrequisition($id,$supplierid,$costcenterid,$vehicleid,$requestedby,$approvedby,$quantity,$unitprice,$odoreading){
            $sql="CALL `sp_savefleetfuelrequisition`({$this->clientid},{$id},{$supplierid},{$costcenterid},{$vehicleid},{$requestedby},{$approvedby},{$quantity},{$unitprice},'{$odoreading}',{$_SESSION['userid']})";
        ...
        public function filterfleetrequisitions($supplierid,$costcenterid,$vehicleid,$startdate,$enddate){
            ...
            $sql="CALL `sp_filterfleetrequisitions`({$this->clientid},{$supplierid},{$costcenterid},{$vehicleid},'{$startdate}','{$enddate}')";
            return $this->getJSON($sql);
        }

        public function getrequisitiondetails($id){
            $sql="CALL `sp_getfleetrequisitiondetails`({$this->clientid},{$id})";
            return $this->getJSON($sql);
        }
        public function approverequisition($id){
            $sql="CALL `sp_fleetapprovefuelrequisition`({$this->clientid},{$id},{$_SESSION['userid']})";
            $this->getData($sql);
            return "success";
        }
    }
