<?php
    ...
    class returns extends db{
        ...
        public function savetempreturns($refno,$productid,$serialno,$quantity){
            $sql="CALL `spsavetempreturns`({$this->clientid},'{$refno}',{$productid},'{$serialno}',{$quantity})";
            $this->getData($sql);
            return "success";
        }

        public function savereturninwards($refno,$receiptno,$narration){
            $sql="CALL `spsavereturninwards`({$this->clientid},'{$refno}','{$receiptno}',{$_SESSION['userid']},'{$narration}')";
        ...
        public function getreturninwards($startdate,$enddate){
            ...
            $sql="CALL `spgetreturninwards`({$this->clientid},'{$startdate}','{$enddate}')";
            return $this->getJSON($sql);
        }

        public function savereturnoutwards($refno,$grnno,$narration){
            $sql="CALL `spsavereturnoutwards`({$this->clientid},'{$refno}','{$grnno}','{$narration}',{$_SESSION['userid']})";
            // return the reference number generated
            $refno=array("referenceno"=>$this->getData($sql)->fetch()['referenceno']);
            return json_encode($refno);
        }
        ...
        public function getreturnoutwards($startdate,$enddate){
            $startdate=$this->mySQLDate($startdate);
            $enddate=$this->mySQLDate($enddate);
            $sql="CALL `spgetreturnoutwards`({$this->clientid},'{$startdate}','{$enddate}')";
            return $this->getJSON($sql);
        }

        public function savereturninwardscollection($id,$collectedby){
            $sql="CALL `spsavereturninwardscollection`({$this->clientid},{$id},'{$collectedby}',{$_SESSION['userid']})";
            $this->getData($sql);
            return "success";
        }

        public function savereturnoutwardsreturn($id){
            $sql="CALL `spsavereturnoutwardsreturn`({$this->clientid},{$id},{$_SESSION['userid']})";
            $this->getData($sql);
            return "success";
        }

        function saveposreturns($outletid,$warehouseid,$paymentmodeid,$reference,$jsondata,$returneditems){
            $sql="CALL `sp_savereturns`({$this->clientid},{$outletid},{$warehouseid},{$paymentmodeid},'{$reference}','{$jsondata}','{$returneditems}',{$_SESSION['userid']})";
            // echo($sql);
