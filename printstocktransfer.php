...
function index()
{
 ...
 //   $pdf->SetDirectionality('rtl'); // Set lang direction for rtl lang
    ...
    // get receipt details
    $sql="CALL spgetstocktransferheader('{$id}')";
    $rst=$db->connect()->query($sql);
...
<div id='printContainer'>
    ...
    <div class="text-center"><b>STOCK TRANSFER FORM</b></div><br/>
   
    <table>
        <tr>
            <td>Source: <?=$data1['sourcename'].' ('.$data1['sourcetype'].")" ?></td>
            <td>Destination: <?=$data1['destinationame'].' ('.$data1['destinationtype'].")" ?></td>
        </tr>
        <tr> 
            <td id='voucherdate'>Date: <b><?=$data1['dateadded'];?></b></td>
            <td id='vouchernumber'>Reference #: <b><?=$data1['referenceno'];?></b></td>
        </tr>
    </table>
   ...
   <hr>
    <table>
        ...
        <?php 
            ...
            $issuedto=$data1['issuedto'];
            $storecontroller=$data1['storecontroller'];
            $sql="CALL spgetstocktransferdetails('{$id}')";
            $rst=$db->connect()->query($sql);
            ...
            foreach($data1 as $dataitem){
                ...
                $overalltotal+=$total;
                echo "<tr><td class='text-left'>". $dataitem['itemcode']."</td><td>".$dataitem['itemname']."</td><td>".$dataitem['serialno']."</td><td class='text-right'><span class='numeric '>".$dataitem['quantity']."</span></td><td class='text-right'><span class='numeric'>".$dataitem['unitprice']."</td><td class='text-right'><span class='numeric'>".$total."</td></tr>";
    ...
    <table>
        ...
        <tr>
            ...
            <td><b>Issued To:</b></td>
            <td><b>Store Controller:</b></td>
           <!-- <td><b>Date: _________________________</b></td>
            <td><b>Signature: _____________________</b></td>-->
        </tr>
        <tr>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
        ...
        <!-- <tr>
            <td id='servedby'><b>Issued To: </b></td>
