$(document).ready(function(){
        ...
        $.getJSON(
            ...
            function(data){
                if(data.length>0){
                    ...
                    errors="Sorry, GRN number not found in the system"
                    returndetailserrordiv.html(showAlert("info",errors))
                }
            },
    ...
    savereturninwards.on("click",function(){
        ...
        if(receiptno==""){
            errors="Please provide GRN number."
            receiptnumberfield.focus()
            ...
            $.post(
                ...
                function(data){
                    ...
                    try{
                        ...
                        errors=`Return outwards completed successfully. Reference# <strong>${data.referenceno}</strong>`
                ...
                function(data){
                    ...
                    if(data.length>0){
                        for(var i=0;i<data.length;i++){
                            ...
                            results+=data[i].delivered==1?`<td>&nbsp;</td>`:`<td><button class='btn btn-xs btn-danger addcreditnote'><span><i class='fas fa-plus-circle fa-sm'></i></span> Credit Note</button></td>`
                            ...
                            results+=data[i].delivered==1?`<td>&nbsp;</td>`:"<td><button class='btn btn-primary btn-xs collected' data-id='"+data[i].id+"'><span><i class='fas fa-people-carry fa-sm'></i></span> Item Collected</button></td></tr>"
                        }
                    }else{
    ...
    // listen to print of Return Outwards Form
    existingreturnstable.on("click",".print",function(e){
        e.preventDefault()
        var referenceno=$(this).attr("data-id"), url=""
        url="../returnoutwards.php?referenceno="+referenceno
    ...
    savecollecteditembutton.on("click",function(){
        var id= itemcollectedidfield.val()
            errors=""
        if(id==""){
