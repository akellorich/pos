<?php
    ...
    class fleet extends db{
        ...
        public function checkvehicleregno($id,$regno){
            $sql="CALL `sp_checkfleetvehicleregno`({$this->branchid},$id,'{$regno}')";
            return $this->getData($sql)->rowCount();
        }

        public function getvehiclebodytypes(){
            $sql="CALL sp_getfleetbodytypes({$this->branchid})";
            return $this->getJSON($sql);
        }

        public function getvehiclefueltypes(){
            $sql="CALL sp_getfleetfueltypes({$this->branchid})";
            return $this->getJSON($sql);
        }

        public function getvehicles(){
            $sql="CALL sp_getfleetvehicles({$this->branchid})";
            return $this->getJSON($sql);
        }

        public function savevehicle($vehicleid,$regno,$bodytypeid, $fueltypeid, $enginerating){
            if(!$this->checkvehicleregno($vehicleid,$regno)){
                $sql="CALL `sp_savefleetvehicle`({$this->branchid},{$vehicleid},'{$regno}',{$bodytypeid},{$fueltypeid},{$enginerating},{$_SESSION['userid']})";
        ...
        public function savefuelrequisition($id,$supplierid,$costcenterid,$vehicleid,$requestedby,$approvedby,$quantity,$unitprice,$odoreading){
            $sql="CALL `sp_savefleetfuelrequisition`({$this->branchid},{$id},{$supplierid},{$costcenterid},{$vehicleid},{$requestedby},{$approvedby},{$quantity},{$unitprice},'{$odoreading}',{$_SESSION['userid']})";
        ...
        public function filterfleetrequisitions($supplierid,$costcenterid,$vehicleid,$startdate,$enddate){
            ...
            $sql="CALL `sp_filterfleetrequisitions`({$this->branchid},{$supplierid},{$costcenterid},{$vehicleid},'{$startdate}','{$enddate}')";
            return $this->getJSON($sql);
        }

        public function getrequisitiondetails($id){
            $sql="CALL `sp_getfleetrequisitiondetails`({$this->branchid},{$id})";
            return $this->getJSON($sql);
        }
        public function approverequisition($id){
            $sql="CALL `sp_fleetapprovefuelrequisition`({$this->branchid},{$id},{$_SESSION['userid']})";
            $this->getData($sql);
            return "success";
        }
    }
