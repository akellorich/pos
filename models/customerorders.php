<?php
    ...
    class customerorders extends db{
        ...
        function savetempcustomerorder($refno,$productid,$quantity,$unitprice){
            $sql="CALL `sp_savetempcustomerorderdetail`({$this->branchid},'{$refno}',{$productid},{$quantity},{$unitprice})";
            $this->getData($sql);
        ...
        function savecustomerorder($refno,$posid,$customerid,$tableid){
            $sql="CALL `sp_savecustomerorder`({$this->branchid},'{$refno}',{$posid},{$customerid},{$tableid},{$this->userid})";
        ...
        function getcustomerorder($orderno){
            $sql="CALL `sp_getcustomerorderdetails`({$this->branchid},'{$orderno}')";
        ...
        function filtercustomerorders($customerid,$tableid,$posid,$waiterid,$startdate,$enddate,$orderstatus){
            ...
            $sql="CALL `sp_filtercustomerorders`({$this->branchid},{$customerid},{$tableid},{$posid},{$waiterid},'{$startdate}','{$enddate}','{$orderstatus}')";
            return $this->getJSON($sql);
        }

        function savetemporderstosettle($refno,$orderid){
            $sql="CALL `sp_savetemporderstosettle`({$this->branchid},'{$refno}',{$orderid})";
            $this->getData($sql);
        }

        function getorderstotal($refno){
            $sql="CALL `sp_getorderstotal`({$this->branchid},'{$refno}')";
            return $this->getJSON($sql);
        }

        function settlecustomerorders($refno){
            $sql="CALL `sp_settleorderpayments`({$this->branchid},'{$refno}',{$this->userid})";
        ...
        function cancelcustomerorder($orderid,$reason){
            $sql="CALL `sp_cancelcustomerorder`({$this->branchid},{$orderid},'{$reason}',{$this->userid})";
            $this->getData($sql);
