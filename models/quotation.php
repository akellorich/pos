<?php
    ...
    class quotation extends db{
        ...
        $trimallowance,$profitmargin,$printing,$freight,$waste,$flutefactor){
            $sql="CALL `sp_savetempquotation`('{$refno}',{$itemid},'{$description}',{$quantity},{$unitprice},{$length},{$width},{$height},{$gsm},{$weight},{$plies},{$jointallowance},
            {$trimallowance},{$profitmargin},{$printing},{$freight},{$waste},{$flutefactor})";
            $this->getData($sql);
            return "success";
        }
        ...
        public function savequotation($id,$refno,$customerid,$terms,$category){
            $sql="CALL `sp_savequotation`({$id},'{$refno}',{$customerid},'{$terms}','{$category}',{$this->userid})";
            // echo $sql;
            return $this->getData($sql)->fetch()['quoteno'];
        }

        public function getquotationterms(){
            $sql="CALL `spgetquotationterms`()";
        ...
        public function filterquotation($customerid,$startdate,$enddate,$quotestatus){
            ...
            $sql="CALL `spfilterquotations`({$customerid},'{$startdate}','{$enddate}','{$quotestatus}')";
            // echo $sql."<br/>";
            return $this->getJSON($sql);
        }
    }
